 ...
 * 구글 프로필 동기화 서비스
 * 
 * Google People API를 통해 사용자의 조직 정보(외부 ID, 부서)를 
 * Keycloak 프로필에 동기화하는 기능을 담당합니다.
 */
public class GoogleProfileSyncService {

    private static final Logger logger = Logger.getLogger(GoogleProfileSyncService.class);
    ...
    public GoogleProfileSyncService(KeycloakSession session) {
        this.session = session;
    }

    /**
     * 구글 연합 신원을 처리하여 프로필 동기화 수행
     */
    public void processGoogleIdentity(FederatedIdentityModel identity, UserModel user, RealmModel realm) {
        if (identity == null) {
